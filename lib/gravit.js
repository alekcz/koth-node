...
function makeText(object) {
	...
	if(object.hasOwnProperty("_stop")) {
...
function makeImage(object) {
	...
	if(object.hasOwnProperty("_eff")) Object.assign(layer, makeEffects(object["_eff"]));
	if(object.hasOwnProperty("_stop")) {
		layer["opacity"] = object["_stop"];
	}
	return layer;
...
function makeEffects(object) {
	var eff = object["$"]
	var effects = {}
	for (var i = eff.length - 1; i >= 0; i--) {
		var tmp = eff[i];
		if(tmp["@"]=="dropShadowEffect"){
			effects["shadowX"] = (tmp["x"] || 0)
			effects["shadowY"] = (tmp["y"] || 0)
			effects["shadowBlur"] = (tmp["r"] || 0)
			effects["shadowColor"] = makeColor(tmp["pat"])
		}
	}
	return effects;
}
...
function makeLayer(object) {
	...
	layer["width"] = object["w"];
	layer["height"] = object["h"];
	layer["x"] = object["x"];//object["trf"][4];
	layer["y"] = object["y"];//object["trf"][5];
	if(object["$"]) {
		layer["children"] = buildChildren(object["$"]);
		var children = layer["children"]
		for(var i = children.length - 1; i >= 0; i--) {
...
function buildLayer(object) {
	if(object)
	{
		...
		if(type == "text") {
			...
			layer = makeEllipse(object);
		}
		else if(type == "effects"){
			layer = makeEffects(object);
		}
		else {
